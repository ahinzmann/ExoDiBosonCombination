...
for mass in masses:
 ...
 outfile="LVJ_cards_8TeV/comb_lvjww8."+str(mass)+".txt"
 print outfile
 f=open(outfile,"w")
 ...
 for l in range(len(fWW)):
   if "rate" in fWW[l]:
     ...
     for s in range(len(fWWsplit)):
 	 ...
 	 numberWW=numberWW/bulkWW[mass]*(0.73+0.08*mass/1000.)*HVTWW[mass] #factor /2 from combinatorics of lnuqq into WW or WZ #+0.19*0.30*HVTWH[mass]*57.7/67.60/2.
       line+="%.5e  " % numberWW
     line+="\n"
     f.write(line)
   else:
