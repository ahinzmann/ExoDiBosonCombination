...
def get_theo_map(sqrts):
   ...
   for line in open('xsect_BulkGProduction_c0p5_correct.txt').readlines():
	split=line.replace("\n","").replace("\t"," ").split(" ")	
	BulkG['CX'].append(float(split[1]))
...
def plot_Asympt_limits(label,signalstrenght):
 ...
 infile = "BulkGResults/higgsCombine%s.Asymptotic.TOTAL.root"%label
 ...
 signal = {}
 signal['ALL13'] = "G_{bulk}"
 signal["ALL8"] = "G_{bulk}"
 signal["xww13"] = "G_{bulk}"
 signal["xjj13"] = "G_{bulk}"
 signal["xww"] = "G_{bulk}"
 signal["xzz"] = "G_{bulk}"
 signal["xjj8"] = "G_{bulk}"
 decay = {}
 ...
 theMap13 = get_theo_map("13")
 theMap8 = get_theo_map("8")
 ...
 for m in mass:
   if signalstrenght:
    ...
    idx = theMap13['M'].index(m)
    scale['ALL13'][m] = theMap13['WW'][idx]+theMap13['ZZ'][idx]
    scale['xww13'][m] = theMap13['WW'][idx]+theMap13['ZZ'][idx]
    scale['xjj13'][m] = theMap13['WW'][idx]+theMap13['ZZ'][idx]
    if m < 3000:
     idx2 = theMap8['M'].index(m)
     scale['ALL8'][m] = theMap8['WW'][idx2]+theMap8['ZZ'][idx2]
     scale['xww'][m] = theMap8['WW'][idx2]+theMap8['ZZ'][idx2]
     scale['xzz'][m] = theMap8['WW'][idx2]+theMap8['ZZ'][idx2]
     scale['xjj8'][m] = theMap8['WW'][idx2]+theMap8['ZZ'][idx2] 
 ...
 theoleg = "#sigma_{TH} #times BR_{"+signal[label]+"#rightarrow "+decay[label]+"} , #tilde{k}=0.5"
 ...
 canv.SaveAs("plots/EXOVVbulkg_"+label+"_UL_Asymptotic_log.root")
 canv.SaveAs("plots/EXOVVbulkg_"+label+"_UL_Asymptotic_log.png")
 canv.SaveAs("plots/EXOVVbulkg_"+label+"_UL_Asymptotic_log.pdf")
 canv.SaveAs("plots/EXOVVbulkg_"+label+"_UL_Asymptotic_log.eps")
...
def compare_Asympt_limits(labels,unblind,bands):
 ...
 legs1={}
 legs1["ALL13"]=[0.38,0.68,0.90,0.84]
 legs1["ALL8"]=[0.38,0.68,0.90,0.84]
    
 legs2={}
 legs2["ALL13"]=[0.38,0.61,0.90,0.65]
 legs2["ALL8"]=[0.38,0.57,0.90,0.65]

 legs3={}
 legs3["ALL13"]=[0.38,0.61,0.90,0.65]
 legs3["ALL8"]=[0.38,0.57,0.90,0.65]
 ...
 files=[TFile.Open("plots/EXOVVbulkg_"+l+"_UL_Asymptotic_log.root") for l in labels]
 ...
 hrl_SM = canv.DrawFrame(0.75,1e-03, x+0.050, 1)    
 if '13' in labels[0]: hrl_SM = canv.DrawFrame(0.75,1e-03, x+0.050, 10)  
 ...
 theoleg = "#sigma_{TH} #times BR_{"+signal[labels[0]]+"#rightarrow "+decay[labels[0]]+"} , #tilde{k}=0.5"
 ...
 canv.SaveAs("plots/EXOVVbulkg_compare_"+labels[0]+"_"+suffix+".png")
 canv.SaveAs("plots/EXOVVbulkg_compare_"+labels[0]+"_"+suffix+".pdf")
 canv.SaveAs("plots/EXOVVbulkg_compare_"+labels[0]+"_"+suffix+".eps")
...
if __name__ == '__main__':
 ...
 # 8 TeV BulkG
 scenarios["ALL8TeV"]=["ALL8","xww","xzz","xjj8"]
