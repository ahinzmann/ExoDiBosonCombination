...
WORKDIR=/afs/cern.ch/user/h/hinzmann/workspace/limit_combination/CMSSW_7_1_5/src/ExoDiBosonCombination/cards/RS1G/${OUTDIR}/comb_${mass}
if [ $ONED -eq 0 ]
    then
    WORKDIR=/afs/cern.ch/user/h/hinzmann/workspace/limit_combination/CMSSW_7_1_5/src/ExoDiBosonCombination/cards/RS1G/${OUTDIR}/comb_${mass}_${width}
fi
...
#change range of scan specifically for RS1->VV with c=0.1
if [ $mass -gt 2000 ]
    then
    maxBoundary=1000
    minBoundary=0.1
...
combine -M $algo -n ${label} -m $mass  -s $myrand -d ${datacard}.txt -H $hint --rMax $maxBoundary --rMin $minBoundary --run blind
