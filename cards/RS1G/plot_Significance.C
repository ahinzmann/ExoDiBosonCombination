...
void plot_Significance(bool unblind, char* scenario){
 ...
 iMH=0;
  ...
  TLegend *l=new TLegend(0.25,0.15,0.65,0.30);
  ...
  TPaveText* cmslabel = new TPaveText( 0.145, 0.953, 0.6, 0.975, "brNDC");
   ...
   char fnam[50];
   sprintf(fnam, "EXOVV_%s_Significance.root", scenario);
   cS->SaveAs(fnam);
   sprintf(fnam, "EXOVV_%s_Significance.eps", scenario);
   cS->SaveAs(fnam);
   sprintf(fnam, "EXOVV_%s_Significance.png", scenario);
   cS->SaveAs(fnam);
   sprintf(fnam, "EXOVV_%s_Significance.pdf", scenario);
   cS->SaveAs(fnam);
}
