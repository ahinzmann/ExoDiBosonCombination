...
for mass in masses:
 ...
 fVV=open("JJ_cards/datacards/CMS_jj_BulkWW_"+str(mass)+"_8TeV_CMS_jj_VV.txt").readlines()
 outfile="JJ_cards/datacards/CMS_jj_BulkWWfix_"+str(mass)+"_8TeV_CMS_jj_VV.txt"
 print outfile
 f=open(outfile,"w")
 ...
 for line in open("xsect_BulkG_WW_c0p5_xsect_in_pb_factor4wrong.txt").readlines():
  split=line.replace("\n","").split("\t")
  BulkWW[int(split[0])]=float(split[1])/4.
 ...
 for l in range(len(fVV)):
  ...
  if "rate" in fVV[l]:
   ...
   for s in range(len(fVVsplit)):
    ...
    if signal:
     numberVV=numberVV*BulkWW[mass]*100.
     newline1+="%.3f  "%pdf_
     newline2+="%.3f  "%scale_
    else:
     newline1+="-  "
