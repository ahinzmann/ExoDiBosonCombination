...
//const bool isSignalStrength = true;
const float intLumi = 19.7;
const string dirXSect = "./";

void plot_golfcourse_Asymptotic(bool unblind = true, char* width = 0, char* scenario = "JAM13", bool isSignalStrength=false);
void setFPStyle();
...
void plot_golfcourse_Asymptotic(bool unblind, char* width, char* scenario, bool isSignalStrength)
{
  ...
  std::cerr << scenario << " is Jamboree " << (strcmp(scenario, "JAM13")==0) << std::endl;
  ...
  string xsect_file_th = dirXSect + "xsec_BulkGProduction_c0p5_correct.txt";
  if (isSignalStrength)xsect_file_th = dirXSect + "theory_signal_strength.txt";
  else xsect_file_th = dirXSect + "xsect_BulkG_WW_c0p5_13TeV.txt";
  ...
  while (xsect_file.good()) {
    ...
    v_mhxs.push_back(mH);
    if (strcmp(scenario, "xjj13zz")==0)
      v_xs.push_back(CS/2.);// *BRZZ2l2q (multyply by BRZZ2l2q only if exp rates in cards are for process X->ZZ->2l2q !)
    else
...
//   }
  ...
  // cout<<"Plotting"<<endl;
  double fr_left = 750.0/1000, fr_down = 1e-3, fr_right = 4050.0/1000, fr_up = 1e1;
  if (isSignalStrength) {
    fr_left = 750.0/1000, fr_down = 3e-1, fr_right = 4050.0/1000, fr_up = 3e4;
  }
  ...
  TString VV = "WW";
  if (strcmp(scenario, "xjj13zz")==0) VV="ZZ";
  hr->SetXTitle("M_{G_{Bulk}} (TeV)");
  hr->SetYTitle("#sigma_{95%} #times BR(G_{Bulk} #rightarrow " + VV + ") (pb)"); // #rightarrow 2l2q
  if(isSignalStrength)
  ...
  gr95_cls->GetXaxis()->SetTitle("M_{G_{Bulk}} (TeV)");
  gr95_cls->GetYaxis()->SetTitle("#sigma_{95%} #times BR(G_{Bulk} #rightarrow " + VV + ") (pb)"); // #rightarrow 2l2q
  ...
  gr68_cls->Draw("3same");
  grmedian_cls->GetXaxis()->SetTitle("M_{G_{Bulk}} (TeV)");
  grmedian_cls->GetYaxis()->SetTitle("#sigma_{95%} #times BR(G_{Bulk} #rightarrow " + VV + ") (pb)"); // #rightarrow 2l2q
  ...
  */
  ...
  if (isSignalStrength) TLegend *leg = new TLegend(.50, .18, .93, .50);
  else if (strcmp(scenario, "JAM13")==0) TLegend *leg = new TLegend(.50, .71, .91, .92,"JJ + l#nuJ channels");
  else if (strcmp(scenario, "xww13")==0) TLegend *leg = new TLegend(.50, .71, .91, .92,"l#nuJ channel");
  else if (strcmp(scenario, "xjj13zz")==0) TLegend *leg = new TLegend(.50, .71, .91, .92,"JJ channel");
  else if (strcmp(scenario, "xjj13WW")==0) TLegend *leg = new TLegend(.50, .71, .91, .92,"JJ channel");
  else  TLegend *leg = new TLegend(.50, .71, .91, .92);
  ...
  //   leg->SetBorderMode(0);
  if (unblind)leg->AddEntry(grobslim_cls, "Asymptotic CL_{S} Observed", "LP");
  leg->AddEntry(gr68_cls, "Asymptotic CL_{S}  Expected #pm 1#sigma", "LF");
  leg->AddEntry(gr95_cls, "Asymptotic CL_{S}  Expected #pm 2#sigma", "LF");
  //if(!isFullCombination) {
  //  leg->AddEntry(grthSM, "#sigma_{TH} x BR(G_{Bulk} #rightarrow " + VV + "), #tilde{k}=0.50", "L"); // #rightarrow 2l2q
  //  leg->AddEntry(grthSM10, "#sigma_{TH} x BR(G_{Bulk} #rightarrow " + VV + "), #tilde{k}=0.20", "L"); // #rightarrow 2l2q
  //}
  //if(isFullCombination) {
    leg->AddEntry(grthSM, "#sigma_{TH} (pp #rightarrow G_{Bulk}), #tilde{k}=0.50", "L");
  //}
  leg->Draw();
  ...
  char fnam[50];
  if (width != 0) {
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_%s.root", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_%s.eps", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_%s.png", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_%s.pdf", scenario, width);
    cMCMC->SaveAs(fnam);
    gPad->SetLogy();
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_%s_log.eps", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_%s_log.png", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_%s_log.pdf", scenario, width);
    cMCMC->SaveAs(fnam);
  } else {
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic.root", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic.eps", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic.png", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic.pdf", scenario);
    cMCMC->SaveAs(fnam);
    gPad->SetLogy();
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_log.eps", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_log.png", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVbulk_%s_UL_Asymptotic_log.pdf", scenario);
    cMCMC->SaveAs(fnam);
  }
