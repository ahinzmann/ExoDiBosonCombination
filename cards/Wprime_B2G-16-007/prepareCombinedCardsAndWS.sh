...
if [ $MASS -ge 800 ] && [ $MASS -le 2500 ]
    ...
    sed -e 's|cards_mu/||g'  -e 's|cards_el/||g'   -e 's|CMS_xwh_prunedmass|CMS_jet_mass|g' -e 's|CMS_xwh_btag_eff|CMS_doubleBtagging|g' -e 's|CMS_xwh_btagger|CMS_xww_btag_eff|g' -e 's|CMS_xwh_trigger_e|CMS_xww_trigger_e|g' -e 's|CMS_xwh_trigger_m|CMS_xww_trigger_m|g' -e 's|CMS_xwh_XS_STop|CMS_xww_XS_STop|g' -e 's|CMS_xwh_XS_VV|CMS_xww_XS_VV|g' < tmp_XWH_card.txt  > ${COMBLVJWH8CARD}
    cd -
