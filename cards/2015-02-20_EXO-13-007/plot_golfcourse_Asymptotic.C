...
void plot_golfcourse_Asymptotic(bool unblind = true, char* width = 0, char* scenario = "EXOZH");
void setFPStyle();
...
//   }
  ...
  if (isFullCombination) {
    fr_left = 750.0, fr_down = 1e-2, fr_right = 2550.0, fr_up = 1e2;
  }
  ...
  gr95_cls->GetYaxis()->SetTitle("#sigma_{95%} #times BR(Z' #rightarrow " + VV + ") [pb]"); // #rightarrow 2l2q
  ...
  grmedian_cls->GetYaxis()->SetTitle("#sigma_{95%} #times BR(Z' #rightarrow " + VV + ") [pb]"); // #rightarrow 2l2q
  ...
  */
  ...
  if(isFullCombination) {
    leg->AddEntry(grthSM, "#sigma_{TH} (pp #rightarrow Z')", "L");
  }
  leg->Draw();
