...
for mass in masses:
 ...
 fVH=open("cards_VH_leptons_13TeV/datacards/XVh_M"+str(mass)+".txt").readlines()
 outfile="cards_VH_leptons_13TeV/datacards/XVhFix_M"+str(mass)+".txt"
 print outfile
 f=open(outfile,"w")
   ...
   if "rate" in fVH[l]:
     ...
     for s in range(len(fVHsplit)):
 	 ...
 	 float(fVHsplit[s])
       ...
       signal=(s in [1,5,9,13,17,21,25,29,33,37])
       signalWH=(s in [1,5,9,13])
       signalZH=(s in [17,21,25,29,33,37])
       numberVH=float(fVHsplit[s])
       if signalWH:
        #print "WH before ",numberVH
        numberVH=numberVH*xsecWH[mass]
        #print "WH after ",numberVH
       if signalZH:
        #print "ZH before ",numberVH
        numberVH=numberVH*xsecZH[mass] 	
        #print "ZH after ",numberVH
       line+="%.5e  " % numberVH
     line+="\n"
     f.write(line)
   else:
