...
for mass in masses:
 ...
 outfile="cards_ZH_8TeV/datacard_"+str(mass)+"_interpolate_VHadapt.txt"
 print outfile
 f=open(outfile,"w")

 xsecWH={}
 xsecWH[mass] = (xsecMap['CX-(pb)'][m]+xsecMap['CX+(pb)'][m])*xsecMap['BRWh'][m]*0.6760*0.0637
 ...
 for l in range(len(fZH)):
   if "rate" in fZH[l]:
     ...
     for s in range(len(fZHsplit)):
 	 ...
 	 numberZH=numberZH*1000.*(xsecZH[mass]+0.95*xsecWH[mass]) # WH has 95% lower Z-tagging efficiency than ZH and twice the cross section
       line+="%.5e  " % numberZH
     line+="\n"
     f.write(line)
   else:
