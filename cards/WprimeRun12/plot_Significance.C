...
void plot_Significance(bool unblind, char* scenario){
 ...
 iMH=0;
  ...
  TPaveText* cmslabel = new TPaveText( 0.145, 0.953, 0.6, 0.975, "brNDC");
   ...
   cS->SaveAs(fnam);
   sprintf(fnam, "EXOVVwprime_%s_Significance.eps", scenario);
   cS->SaveAs(fnam);
   sprintf(fnam, "EXOVVwprime_%s_Significance.png", scenario);
   cS->SaveAs(fnam);
   sprintf(fnam, "EXOVVwprime_%s_Significance.pdf", scenario);
   cS->SaveAs(fnam);
}
