...
for mass in masses:
 ...
 outfile="LLJ_cards_8TeV/"+str(mass)+"/comb_lljwzh8."+str(mass)+".txt"
 print outfile
 f=open(outfile,"w")
 ...
 #tot = (xsecMap['CX-(pb)'][m]+xsecMap['CX+(pb)'][m])*(xsecMap['BRZW'][m]+xsecMap['BRWh'][m])+xsecMap['CX0(pb)'][m]*(xsecMap['BRWW'][m]+xsecMap['BRhZ'][m])
 ...
 for l in range(len(fZZ)):
  ...
  if "rate" in fZZ[l]:
   ...
   for s in range(len(fZZsplit)):
    ...
    if signal:
     #print tot/(HVTWZ[mass]*67.60/69.91+0.27*HVTZH[mass]*57.7/69.91)
     if mass > 700:
      numberZZ=numberZZ/bulkZZ[mass]*0.8*(HVTWZ[mass]*67.60/69.91/2.+0.27*HVTZH[mass]*57.7/69.91/2.) #factor /2 from combinatorics of llqq into ZZ or WZ
