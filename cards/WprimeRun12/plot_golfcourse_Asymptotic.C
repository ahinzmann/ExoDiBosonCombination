...
void plot_golfcourse_Asymptotic(bool unblind = false, char* width = 0, char* scenario = "JAM13", bool isSignalStrength = false);
void setFPStyle();
...
void plot_golfcourse_Asymptotic(bool unblind, char* width, char* scenario, bool isSignalStrength)
{
  ...
  else xsect_file_th = dirXSect + "theory_HVT_WZ_13TeV.txt";
...
//   }
  ...
  if (isSignalStrength) {
    fr_left = 750.0/1000, fr_down = 1e-2, fr_right = 4050.0/1000, fr_up = 1e2;
  }
  ...
  hr->SetYTitle("#sigma_{95%} #times BR(W' #rightarrow " + VV + ") (pb)"); // #rightarrow 2l2q
  if(isSignalStrength)
  ...
  gr95_cls->GetYaxis()->SetTitle("#sigma_{95%} #times BR(W' #rightarrow " + VV + ") (pb)"); // #rightarrow 2l2q
  ...
  grmedian_cls->GetYaxis()->SetTitle("#sigma_{95%} #times BR(W' #rightarrow " + VV + ") (pb)"); // #rightarrow 2l2q
  ...
  */
  ...
  else if (strcmp(scenario, "xjj13")==0) TLegend *leg = new TLegend(.50, .71, .91, .92,"JJ channel");
  ...
  char fnam[50];
  if (width != 0) {
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_%s.root", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_%s.eps", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_%s.png", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_%s.pdf", scenario, width);
    cMCMC->SaveAs(fnam);
    gPad->SetLogy();
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_%s_log.eps", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_%s_log.png", scenario, width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_%s_log.pdf", scenario, width);
    cMCMC->SaveAs(fnam);
  } else {
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic.root", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic.eps", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic.png", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic.pdf", scenario);
    cMCMC->SaveAs(fnam);
    gPad->SetLogy();
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_log.eps", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_log.png", scenario);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVVwprime_%s_UL_Asymptotic_log.pdf", scenario);
    cMCMC->SaveAs(fnam);
  }
